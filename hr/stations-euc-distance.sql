...
INSERT INTO station (id, city, state, LAT_N, LONG_W) VALUES (6, 'Denver', 'CO', 42.8781, -87.6298);
    ...
    ROUND(
        CASE 
            WHEN total_rows % 2 = 1 THEN 
                (SELECT LAT_N FROM ranking WHERE row_num = (total_rows + 1) / 2)
            ELSE 
                (SELECT AVG(LAT_N) FROM ranking WHERE row_num IN (total_rows / 2, total_rows / 2 + 1))
        END, 
    4) AS median_LAT_N
FROM ranking ;
